# Hotfix - Modal scrollable + editar lead (feedback Carolina A primer uso real)
...
$files = @(
  ...
  'supabase/migrations/20260504_rpc_actualizar_lead_captacion.sql'
)
foreach ($f in $files) {
  if (Test-Path $f) {
...
git commit -F COMMIT_MSG_FIX_MODAL_Y_EDITAR.txt
if ($LASTEXITCODE -ne 0) { exit 1 }
