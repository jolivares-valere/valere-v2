...
- **S0.2** Ampliar worker `datadis-sync`: por CUPS autorizado traer y **persistir** contrato + curva (12–24 meses) + maxímetro. Idempotente, no-destructivo, batches. Incluye endurecimientos de la auditoría 2026-07-13: (B3) detectar colisión `base20` en ambos lados y emitir incidencia `base20_duplicado` en vez de sobrescribir; (B4) cambiar el refresco de incidencias de delete+insert a **upsert ON CONFLICT** sobre `uq_datadis_incidencias_dedupe`; **comprobar el error del insert de incidencias** (hoy v8 lo ignora); **setear `updated_at`** al upsertear `datadis_contratos`; **normalizar `fecha_inicio` nula** (afecta al UNIQUE de contratos); **ventana rodante de 24 meses** por CUPS (+ job de purga/agregado de lo anterior, ver S0.5). · **M**
