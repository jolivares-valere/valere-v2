...
> Última actualización: 2026-04-27 por Cowork (cierre de sesión larga: Holded Fase 0+1 aplicado prod + auditoría CRM ZocoEnergía + protocolo de trabajo continuo + backlog priorizado). **Sesión arranca chat nuevo "Driver Cowork" siguiendo modelo del protocolo**. Plan D acordado con Juan: (1) redactar `docs/PLAN_INTEGRACION_DATADIS.md` con mini-spec arquitectura proxy backend + RGPD consentimiento + plan de fases 4-6 sem dev + 2-4 sem trámite admin paralelo; (2) Juan inicia trámite alta empresa habilitada Datadis (papeleo bloqueante para Sprint C, lento, hacer en paralelo); (3) lanzar Sprint A "Quick Wins post-auditoría ZocoEnergía" como 4 sub-agentes Implementador en paralelo con git worktree isolation desde el chat Driver: Kanban Oportunidades drag&drop (rama `claude/kanban-oportunidades`, 1-2 sem, `@dnd-kit/core` ya en stack), Importador XLSX tarifas (rama `claude/importador-xlsx-tarifas`, 1 sem, `sheet-js` ya en stack), Audit log básico (rama `claude/audit-log`, 1 sem, cierra M-03 audit seguridad), Botón "Atrás contextual" prominente (rama `claude/back-button-contextual`, 3 días). **Documentos vivos creados este chat**: `docs/PROTOCOLO_TRABAJO_CRM.md` (modelo "1 chat Driver + sub-agentes" con 5 perfiles de actor + plantillas de arranque + cadencia + reglas), `docs/BACKLOG_CRM.md` (memoria viva con secciones priorizadas: Esta semana / Próximo mes / Trimestre / Futuro / Seguridad / Deuda técnica / Operativos / Sin priorizar / Hecho — pre-poblado con 30+ items del roadmap auditor + audit seguridad pendientes + deuda técnica), `docs/AUDIT_CRM_ZOCOENERGIA_2026-04-27.md` (informe completo del agente Browser, 12 secciones, calificación design system Zoco 5,3/10, top 10 features que Valere no tiene, top 5 fortalezas Valere, 3 patrones UX a copiar, roadmap completo Quick wins/Medio/Largo, hallazgos seguridad/RGPD/deuda técnica). **Estado prod**: CRM `gtphkowfcuiqbvfkwjxb` sano, Holded Fase 1 aplicado y validado smoke test E2E (cron HTTP 200 `{"processed":0}`), `holded_config.enabled=false` (estado seguro, cron skip silencioso). **Pendientes Juan operativos**: (1) repo en local — el push del sprint Holded Fase 1 está hecho, falta abrir PR `claude/holded-integration → main` SIN mergear; (2) verificar NIF correcto de PAZ Y BIEN 5002AP cuando puedas; (3) iniciar trámite admin Datadis cuando el chat Driver te indique los pasos exactos. **Chats abiertos a archivar**: la sesión Cowork actual (Holded Fase 0+1 + audit + auditoría Zoco + protocolo) que cerró 2026-04-27 — todo persistido en `docs/`, no se pierde nada al cerrar.

> Última actualización (entrada previa): 2026-04-27 por Cowork (Sprint Holded Fase 0 + Fase 1 — **APLICADO EN PROD vía agente Browser**, sólo Edge Functions deploy + commit/PR pendientes Juan). **Migrations aplicadas**: `20260427_holded_data_audit.sql` y `20260427_holded_infrastructure.sql`. Verificación independiente Cowork via MCP execute_sql 2026-04-27 15:15 UTC: holded_audit_resumen muestra 24 empresas activas / 24 con NIF / 23 CIF + 1 NIF persona / 23 checksum válidos + 1 inválido (PAZ Y BIEN excluida del sync) / 0 INVALID format; holded_config singleton enabled=false mode=dry_run productos_sync_mode=read excluded_nifs=['G12345678','TEST_VALERE_NOSINCRONIZAR'] updated_at=2026-04-27 15:11:39+00; 5 tablas creadas (config=1, sync_queue=0, integration_logs=0, conflicts=0, sync_state=13 entidades seedeadas); 18 policies granulares con roles canónicos master/manager/consultant/client (config=4, queue=4, logs=2 insert+select inmutable, conflicts=4, state=4); pg_cron job `holded_worker_5min` schedule `*/5 * * * *` active=true; pg_net extension 0.20.0 instalada vía Dashboard UI; HOLDED_CRON_SECRET en `vault.secrets` (length=73 = uuid-uuid) created_at=2026-04-27 14:09:30 — Juan debe sincronizar el mismo valor a Edge Function Secrets desde Settings → Edge Functions; `holded_dispatch_worker()` ejecuta y devuelve 0 con 0 logs emitidos (skip silencioso correcto porque enabled=false). **Block D limpieza datos**: 3 empresas soft-deleted con `deleted_at=2026-04-27 15:11:25+00` (Empresa Test SA, Industrias Valere Test S.L., la primera prueba sl) + nota automática insertada en `notas`; PAZ Y BIEN 5002AP (id `e95025b4-...`) NO borrada — activa pero excluida del sync hasta que Juan verifique el NIF correcto, NIF normalizado `G12345678` añadido a `holded_config.excluded_nifs` y nota explicativa en `empresas.notas` referenciando `docs/AUDIT_DATOS_VALERE_PRE_HOLDED_2026-04-27.md`. **Estado prod CRM (`gtphkowfcuiqbvfkwjxb`) = sano y sin cambios destructivos**: el cron corre cada 5 min pero el worker hace skip silencioso por `holded_config.enabled=false`, zero impacto hasta activación manual. **Pendientes Juan (4 puntos)**: (1) reparar `.git/index` corrupto (`RUNBOOK.ps1 -OnlyRepair`) + crear rama `claude/holded-integration` desde main + ejecutar `RUNBOOK_HOLDED_FASE01.ps1` para stage/commit/push del sprint; (2) añadir `HOLDED_CRON_SECRET` a Edge Function Secrets en Dashboard (mismo valor que en Vault — Browser cumplió la regla de no exponerlo en chat así que Juan necesita re-generarlo desde vault.decrypted_secrets o copiar del Dashboard de Vault directamente); (3) deploy 3 Edge Functions con CLI Supabase desde Windows: `supabase functions deploy holded-worker --no-verify-jwt --project-ref gtphkowfcuiqbvfkwjxb` + `supabase functions deploy notify-integration-error --project-ref gtphkowfcuiqbvfkwjxb` (el `_shared/holded-client.ts` se sube con cualquiera de los dos); (4) crear PR `claude/holded-integration` → `main` SIN mergear; smoke test opcional `/admin?tab=holded`. Próxima sesión Cowork = Fase 2 catálogos (pull-only Holded→CRM, registry de handlers en holded-worker se puebla con primer entry `producto:pull`). Detalle handoff completo en `.cowork/outbox/2026-04-27T13-30-00-holded-fase01-handoff.md` y sesión en `docs/SESIONES/2026-04-27-holded-fase0-fase1.md`.

> Última actualización (entrada previa, ahora superada): 2026-04-27 por Cowork (Sprint Holded Fase 0 + Fase 1 — código + DDL escritos, NADA aplicado en prod). **Sprint arrancado tras confirmación Juan**: C-01 (passwords DB) ya cerrado el mismo día (Juan reseteó ambos durante Fase 2 unificación, riesgo residual cero), M-06 (Leaked Password Protection) decidido NO aplicar por requerir Plan Pro Supabase ($25/mes) — riesgo residual aceptado dada la complejidad mínima 12 chars + lowercase+upper+digits+symbols + secure email/password change. La cabecera de las dos updates anteriores que decían "NO arrancar Holded hasta C-01 + M-06" queda invalidada por esta decisión documentada. **Entregables Fase 0**: (a) `supabase/migrations/20260427_holded_data_audit.sql` con función IMMUTABLE `valida_nif_cif(text)` (algoritmo letra control NIF/NIE + dígito CIF + prefijos VAT intracom 28 países UE), `clasifica_nif_cif(text)`, `normaliza_nif_cif(text)`, y 3 vistas `holded_audit_empresas/resumen/duplicados_nif` con security_invoker=on; (b) `docs/AUDIT_DATOS_VALERE_PRE_HOLDED_2026-04-27.md` reporte ejecutivo: 27 empresas activas (post-Fase2), 26/27 con NIF, 25/26 CIFs + 1 NIF persona, 23/26 checksum válido (88%), 3 con NIF malformado (2 tests obvios + 1 cliente real "PAZ Y BIEN 5002AP" Valencia que requiere verificación Juan), 1 sin NIF (test descartable), 0 duplicados normalizados (Fase 2 dedupó OK), 100% país ISO-2 'ES', columna `provincia` casi vacía (1/27, no bloqueante), `tipo` 100% NULL (mapper asignará default 'client'). Conclusión: NO requiere migration de limpieza destructiva. **Entregables Fase 1**: (a) `supabase/migrations/20260427_holded_infrastructure.sql` con 3 cols holded_id/holded_etag/holded_synced_at en empresas/contactos/oportunidades/actividades/contratos (+ 5 índices únicos parciales), 5 tablas nuevas `holded_config` (singleton seed enabled=false dry_run productos_sync_mode=read excluded_nifs=['TEST_VALERE_NOSINCRONIZAR']) + `holded_sync_queue` (idempotency_key UNIQUE, estados pending/processing/done/error/skipped/dead_letter, backoff exponencial) + `holded_integration_logs` (insert-only inmutable) + `holded_conflicts` + `holded_sync_state` (13 entidades seedeadas), RLS habilitado + 18 policies granulares con roles canónicos master/manager/consultant/client (audit C-03), helpers SQL `is_master()` (faltaba), `holded_mask_nif()`/`holded_mask_iban()` (RGPD), `holded_get_config()`, `holded_enqueue()` (SECURITY DEFINER idempotente), `holded_dispatch_worker()` (lee CRON_SECRET de Vault y llama Edge Function via pg_net), pg_cron job `holded_worker_5min` programado (skip silencioso si enabled=false), `CREATE EXTENSION pg_net`. (b) Edge Functions Deno: `supabase/functions/_shared/holded-client.ts` con auth header `key:`, token bucket 5 req/s, retry exponencial 1/2/4/8/16/32s max 6, timeout 15s, masking IBAN/NIF en logs, etag MD5 determinista; `supabase/functions/holded-worker/index.ts` valida X-Cron-Secret, lee config, batch 25 items, registry de handlers extensible (vacío en Fase 1, Fase 2 añadirá pull catálogos), backoff dinámico hasta dead_letter; `supabase/functions/notify-integration-error/index.ts` (Resend, mismo patrón notify-admin-pending-user, sólo master). (c) Frontend: `src/core/integrations/holded/validators.ts` (gemelo JS-side de las funciones SQL) + `src/core/integrations/holded/validators.test.ts` con 35+ tests Vitest (paridad 1:1 con tests SQL). (d) UI: `src/features/admin/components/HoldedTab.tsx` (estado integración, 6 métricas cola, salud datos pre-sync desde holded_audit_resumen, último error, toggle enabled con ConfirmDialog para master, botón "Notificar errores ahora", placeholder "Pull catálogos Fase 2"). Tab "Holded" añadido a `AdminPage.tsx` sin tocar tabs existentes. **DRY-RUN PROD VALIDADO**: 10 tests unitarios `valida_nif_cif()` PASS contra prod via BEGIN/ROLLBACK; auditoría real sobre las 27 empresas reproducible; migration infraestructura aplica limpia (15 cols dominio + 5 tablas + 18 policies + 0 errores). **NADA APLICADO en prod**. **Pendiente Juan (en orden)**: (1) reparar `.git/index` corrupto (error format 0x00730000) con `RUNBOOK.ps1 -OnlyRepair`, (2) crear rama `claude/holded-integration` desde main, (3) ejecutar `RUNBOOK_HOLDED_FASE01.ps1` (raíz repo), (4) revisar dry-run de las 2 migrations, dar OK, aplicar via MCP `apply_migration`, (5) generar `HOLDED_CRON_SECRET` (gen_random_uuid) y meterlo en Vault Y en Edge Function Secrets, (6) deploy 3 Edge Functions (`holded-worker` con verify_jwt=false, `notify-integration-error` con verify_jwt=true, `_shared/holded-client.ts` se sube como dependencia), (7) decidir destino de las 4 empresas problemáticas (3 tests + PAZ Y BIEN 5002AP), (8) crear PR sin merge. Detalle handoff completo en `.cowork/outbox/2026-04-27T<HHMMSS>-holded-fase01-handoff.md` y `docs/SESIONES/2026-04-27-holded-fase0-fase1.md`.

> Última actualización: 2026-04-27 por Cowork (auditoría seguridad completa + hardening crítico) — **Auditoría seguridad solicitada por Juan ANTES de continuar con Holded**. Informe completo en `docs/AUDIT_SEGURIDAD_2026-04-27.md`: 5 hallazgos críticos, 8 medios, 11 puntos buenos. Memoria de proyecto actualizada (`memory/project_valere_security_priority.md`) — toda sesión Cowork futura prioriza seguridad. Acciones aplicadas hoy via `apply_migration security_hardening_critical_v2_2026_04_27`: (1) C-02 RLS hardening incidencias + renovaciones (eran USING(true) FOR ALL = cualquier authed CRUD todo → ahora 4 policies granulares cada una, INSERT/UPDATE creador-o-asignado-o-manager+, DELETE manager+); (2) C-05 documentos + eventos cambio role `{public}` → `{authenticated}` (defensa profundidad); (3) M-08 REVOKE EXECUTE de funciones SECURITY DEFINER de anon/public (handle_new_user, get_user_rol, is_approved, is_manager_or_above, admin_reject_user, cleanup_pending_users), GRANT explícito solo a authenticated. Pendiente Juan: (a) C-01 rotar passwords DB Supabase via Browser (circularon hoy por chat), (b) M-06 toggle Leaked Password Protection en Auth Dashboard. **NO arrancar sprint Holded hasta C-01 + M-06 completos**. Próximas acciones de seguridad post-hardening crítico: C-03 (unificar roles, decisión Juan), C-04 (forzar encriptación passwords Datadis, sprint 2h), M-01 (MFA Supabase), M-02 (network restrictions), M-03 (audit log), M-05 (backup offsite a R2). Política RGPD pendiente formalizar antes de Holded (registro tratamientos + Holded como sub-encargado).

> Última actualización: 2026-04-27 por Cowork (RLS hardening + decisiones cutover/PDFs) — **2 cierres adicionales**: (1) Cutover URL Potencias-app DECISIÓN: posponer (B). Razón: las views compat actuales solo cubren 4 de las ~10+ tablas que Potencias-app espera (clients, supplies, profiles, power_requests, etc.). Sin views completas + INSTEAD OF triggers el cutover rompería frontend. Pendiente para sprint dedicado. Estado actual estable: CRM = consolidado interno + Potencias-app = satélite original (rollback URL aplicado ayer 26-04 ya correcto). (2) PDFs Storage DECISIÓN: posponer. Razón: los ~100 PDFs (~15 MB) siguen accesibles vía buckets Potencias mientras el satélite esté vivo. Migrar cuando se decida pausar Potencias. (3) RLS hardening 8 tablas Potencias-side APLICADO via migration `rls_hardening_8_tables_potencia_side_2026_04_27`: 8 USING(true) heredadas → policies granulares (select abierto authed, insert/update creador-o-manager+, delete manager+). Caso especial alertas_update_leida queda con USING(true) por decisión Juan (cualquier authed marca leída/no-leída). Patrón documentado: confirmadas las 3 preguntas pendientes con SÍ por Juan. (4) Side-effect detectado y arreglado: 7 tablas `_migration_*_map` (auditoría Fase 2) estaban expuestas a PostgREST sin RLS → 7 ERRORs `rls_disabled_in_public`. Migration `rls_migration_map_tables_manager_only_2026_04_27` aplicada: RLS enabled + policy "manager+ ALL". Resumen advisors post: de 8 WARN rls_policy_always_true → 1 (intencional alertas), 0 ERROR rls_disabled. Quedan 12 WARN advisorios sobre SECURITY DEFINER functions exposed (aceptables porque validan internamente) + 1 WARN auth_leaked_password_protection (toggle dashboard pendiente). **Pendientes acumulados restantes**: rotar passwords DB (circularon chat), sprint cutover Potencias views completas, push acumulado sprints 5-8, Vercel 301, sprint Holded (7 decisiones + API key + nueva sesión Cowork).

> Última actualización: 2026-04-27 por Cowork (FASE 2 UNIFICACIÓN POTENCIAS COMPLETADA EN PROD) — **526 filas migradas Potencias→CRM con dedupe + cero huérfanos**. Migration completa de Postgres-to-Postgres cross-region (eu-central-1 → eu-west-1) ejecutada con éxito. Datos del negocio consolidados en CRM canónico (`gtphkowfcuiqbvfkwjxb`). Métricas finales: empresas 3→27 (+24 dedupe por CIF de 30 clients), cups 1→73 (+72 dedupe por CUPS de 75 supplies), expedientes 0→41, ciclos 0→41, solicitudes_potencia 0→41, savings_calculations 0→41 (89.061,64€ ahorro pendiente migrado), comunicaciones_cliente 0→31, documentos 0→98 (polimórfico: 70 client + 27 expediente + 1 general), comercializadora_docs 0→1, status_log 0→91, email_templates 0→2, comercializadoras 6→8 (+2 nuevas dedupe por nombre), precios_regulados_boe 29→47 (+18 dedupe por period+tariff_type+valid_from). Verificación integridad: 0 huérfanos en 9 FK críticas. Backups full pre-Fase2 en `C:\Users\joliv\valere-backups\`. Schema staging `_potencia_staging` eliminado tras migración OK. Side-effects schema CRM: `documentos_tipo_check` extendido a 10 valores (+`estudio_ahorro`, `email_enviado`), `documentos_entidad_tipo_check` extendido a 6 valores (+`expediente`, `general`). Tablas `_migration_*_map` (7) persisten para auditoría. **Pendiente Juan**: (1) smoke test UI Fase 2 en `valere-v2.pages.dev/empresas` (5 min), (2) decisión cutover URL Potencias-app, (3) migrar PDFs Storage opcional (~100 PDFs, 15 MB), (4) RLS hardening 8 tablas (draft existe), (5) rotar passwords DB Supabase circularon por chat. Detalle completo en `.cowork/outbox/2026-04-27T09-38-36-fase2-unificacion-COMPLETADA.md`.

> Última actualización: 2026-04-26 por Cowork (cierre smoke test signup + cierre rollback URL Potencias) — **SPRINT SIGNUP COMPLETAMENTE CERRADO + POTENCIAS-APP RESTAURADA**. (1) Smoke test E2E del flujo signup verificado en producción (`valere-v2.pages.dev`): signup `juanolivarespena+signup4@gmail.com` → /pending-approval → email a admin recibido en `jolivares@valereconsultores.com` (cadena Resend con key v3 limpia, dominio `valereconsultores.com` verified) → aprobación admin → login del client → dashboard OK con guard de ruta funcionando (cliente NO ve /admin). Tiempo total signup→login: ~17 min. (2) Cloudflare Pages "valere-gestion-potencias": rollback de `VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY` desde proyecto CRM (donde apuntaban mal) a proyecto satélite original `alesfvxqtwlrwlmkoosg.supabase.co`. Deploy 45s. App restaurada con 23 clientes activos + 69 CUPS + datos del negocio intactos (89.061,64€ ahorro pendiente visible de nuevo). (3) Detalle UX detectado para backlog futuro: el AdminPage tiene 2 caminos para aprobar usuarios — tab "Usuarios" (legacy, simple UPDATE silencioso) y tab "Pendientes" (sprint nuevo, dispara email via `notify-user-approval-decision`). Consolidar en próximo sprint. **Pendientes orden recomendado**: (a) Fase 2 unificación Potencias (60 min con backups via pg_dump+psql), (b) push acumulado sprints 5-8 vía `RUNBOOK.ps1` maestro, (c) decisión Vercel 301, (d) tomar 7 decisiones Holded + generar API Key + Vault + lanzar nueva sesión Cowork con prompt `docs/COWORK_PROMPT_HOLDED_INTEGRATION.md`.

> Última actualización: 2026-04-26 por Cowork (planificación integración Holded) — **Plan Holded ↔ Valere CRM listo y adaptado al stack real**. (1) `docs/HOLDED_INFORME_BROWSER_2026-04-26.md` — informe original del agente Browser preservado. (2) `docs/PLAN_INTEGRACION_HOLDED.md` — plan adaptado a Supabase + Edge Functions + pg_cron + Vault (NO middleware externo / NO Redis / NO BullMQ). 8 fases con estimación 18-25 días persona. Mapeo a tablas existentes + 5 tablas nuevas de infraestructura (`holded_config`, `holded_sync_queue`, `holded_integration_logs`, `holded_conflicts`, `holded_sync_state`). Decisiones de fuente de verdad por entidad. Riesgos vs trabajos en curso identificados (Fase 2 unificación debe ir ANTES). Coste infra adicional 0€. (3) `docs/COWORK_PROMPT_HOLDED_INTEGRATION.md` — prompt de arranque para nueva sesión Cowork dedicada (no Claude Desktop, porque Cowork tiene MCP Supabase + acceso repo). (4) `.cowork/outbox/2026-04-26T21-09-34-holded-planning-handoff.md` — handoff completo. **Pendiente Juan**: cerrar trabajos en curso (smoke test signup, Fase 2 unificación, Cloudflare Potencias) + tomar 7 decisiones (sección 6 del plan) + generar API Key Holded "Valere CRM Integration" + meterla en Vault + lanzar nueva sesión Cowork. NO hay código tocado en este sprint, todo es planificación.
