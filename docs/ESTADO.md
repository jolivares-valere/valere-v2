...
> **Última actualización: 2026-06-26 — 🟢 HITO DATADIS PARTNER: convenio de Partner FIRMADO por ambas partes. Datadis confirma por email (Adrián, dpo@datadis.es, 26-jun 09:35) que «ya deberíais tener activo el partner en Datadis» (la activación del acceso puede tardar la mañana en propagarse). Esto desbloquea la API privada/agregada: acceso a CUPS de clientes SIN cuenta propia en Datadis, mediante autorización firmada custodiada por Valere. Origen: gestión por email (no toca repo). Pendiente: (1) verificar acceso real en datadis.es + pestaña "Soy partner" ya operativa; (2) montar Vía A en CRM (PDF autorización autorrellenado + archivo en empresa); (3) protocolo de auditorías Datadis (2/año); (4) carpeta Drive `Datadis Partner/` + onboarding Julia. Ver `docs/SESIONES/2026-06-26-resumen.md`.**
